...
from wheel_legged_gym.utils.math import quat_slerp, standardize_quaternion, quat_rotate_inverse_np
...
AMP_MODE_BACKWARD = 2
AMP_MODE_YAW = 3
AMP_MODE_STRAFE_LEFT = 4
AMP_MODE_STRAFE_RIGHT = 5
AMP_MODE_TURN_LEFT = 6
AMP_MODE_TURN_RIGHT = 7

AMP_MODE_NAMES = {
    AMP_MODE_OTHER: "stand_other",
    AMP_MODE_FORWARD: "forward",
    AMP_MODE_BACKWARD: "backward",
    AMP_MODE_YAW: "yaw",
    AMP_MODE_STRAFE_LEFT: "strafe_left",
    AMP_MODE_STRAFE_RIGHT: "strafe_right",
    AMP_MODE_TURN_LEFT: "turn_left",
    AMP_MODE_TURN_RIGHT: "turn_right",
}
AMP_MODE_IDS = tuple(AMP_MODE_NAMES.keys())


def _amp_name_number_to_float(token):
...
def parse_amp_command_from_name(motion_file):
    name = os.path.basename(motion_file).lower()

    yaw_match = re.search(r"yaw_(left|right)_w([0-9]+p[0-9]+)", name)
    if yaw_match is not None:
        speed = _amp_name_number_to_float(yaw_match.group(2))
        wz = speed if yaw_match.group(1) == "left" else -speed
        return 0.0, 0.0, wz

    strafe_match = re.search(r"strafe_(left|right)_v([0-9]+p[0-9]+)", name)
    if strafe_match is not None:
        speed = _amp_name_number_to_float(strafe_match.group(2))
        vy = speed if strafe_match.group(1) == "left" else -speed
        return 0.0, vy, 0.0

    turn_match = re.search(
        r"turn_(left|right)_vx(n?[0-9]+p[0-9]+)_w([0-9]+p[0-9]+)",
        name,
    )
    if turn_match is not None:
        vx = _amp_name_number_to_float(turn_match.group(2))
        speed = _amp_name_number_to_float(turn_match.group(3))
        wz = speed if turn_match.group(1) == "left" else -speed
        return vx, 0.0, wz

    linear_match = re.search(
        r"(forward|backward)_v([0-9]+p[0-9]+)_(w|wn)([0-9]+p[0-9]+)",
        name,
    )
    if linear_match is not None:
        speed = _amp_name_number_to_float(linear_match.group(2))
        vx = speed if linear_match.group(1) == "forward" else -speed
        wz = _amp_name_number_to_float(linear_match.group(4))
        if linear_match.group(3) == "wn":
            wz = -wz
        return vx, 0.0, wz
    ...
    if grid_match is not None:
        vx = _amp_name_number_to_float(grid_match.group(1))
        vy = _amp_name_number_to_float(grid_match.group(2))
        wz = _amp_name_number_to_float(grid_match.group(4))
        if grid_match.group(3) == "wn":
            wz = -wz
        return vx, vy, wz
...
def infer_amp_motion_mode_from_command(
    vx,
    vy,
    wz,
    lin_threshold=0.10,
    yaw_threshold=0.20,
    yaw_lin_threshold=0.20,
):
    abs_vx = abs(vx)
    abs_vy = abs(vy)
    abs_wz = abs(wz)
    lin_xy = (vx * vx + vy * vy) ** 0.5

    if abs_wz > 1e-6 and lin_xy <= 1e-6:
        return AMP_MODE_YAW
    if abs_wz > 1e-6:
        return AMP_MODE_TURN_LEFT if wz > 0.0 else AMP_MODE_TURN_RIGHT

    if abs_vx >= abs_vy:
        if vx > lin_threshold:
            return AMP_MODE_FORWARD
        if vx < -lin_threshold:
            return AMP_MODE_BACKWARD
    else:
        if vy > lin_threshold:
            return AMP_MODE_STRAFE_LEFT
        if vy < -lin_threshold:
            return AMP_MODE_STRAFE_RIGHT

    return AMP_MODE_OTHER


def infer_amp_motion_mode_from_name(
    motion_file,
    lin_threshold=0.10,
    yaw_threshold=0.20,
    yaw_lin_threshold=0.20,
    ...
    name = os.path.basename(motion_file).lower()
    command = parse_amp_command_from_name(name)
    if command is not None:
        return infer_amp_motion_mode_from_command(
            *command,
            lin_threshold=lin_threshold,
            yaw_threshold=yaw_threshold,
            yaw_lin_threshold=yaw_lin_threshold,
        )

    if "walk_turn_left" in name or "turn_left" in name:
        return AMP_MODE_TURN_LEFT
    if "walk_turn_right" in name or "turn_right" in name:
        return AMP_MODE_TURN_RIGHT
    if "strafe_left" in name:
        return AMP_MODE_STRAFE_LEFT
    if "strafe_right" in name:
        return AMP_MODE_STRAFE_RIGHT
    if "walk_forward" in name or "forward" in name:
        return AMP_MODE_FORWARD
    if "walk_backward" in name or "backward" in name:
        return AMP_MODE_BACKWARD
    if "spin_inplace" in name or "yaw_left" in name or "yaw_right" in name:
        return AMP_MODE_YAW
    if "stand" in name:
        return AMP_MODE_OTHER
...
def classify_amp_motion_modes(
    ...
    yaw_mask = (abs_wz >= yaw_threshold) & (lin_xy < yaw_lin_threshold)
    turn_mask = (abs_wz >= yaw_threshold) & (~yaw_mask)
    linear_mask = (~yaw_mask) & (~turn_mask)
    x_dominant = abs_vx >= abs_vy
    y_dominant = abs_vy > abs_vx

    forward_mask = (vx > lin_threshold) & x_dominant & linear_mask
    back_mask = (vx < -lin_threshold) & x_dominant & linear_mask
    strafe_left_mask = (vy > lin_threshold) & y_dominant & linear_mask
    strafe_right_mask = (vy < -lin_threshold) & y_dominant & linear_mask
    turn_left_mask = turn_mask & (wz > 0.0)
    turn_right_mask = turn_mask & (wz < 0.0)

    labels[forward_mask] = AMP_MODE_FORWARD
    labels[back_mask] = AMP_MODE_BACKWARD
    labels[yaw_mask] = AMP_MODE_YAW
    labels[strafe_left_mask] = AMP_MODE_STRAFE_LEFT
    labels[strafe_right_mask] = AMP_MODE_STRAFE_RIGHT
    labels[turn_left_mask] = AMP_MODE_TURN_LEFT
    labels[turn_right_mask] = AMP_MODE_TURN_RIGHT
    return labels
...
class AMPLoader:
    ...
    def __init__(
            ...
            motion_files=glob.glob(f'{WHEEL_LEGGED_GYM_ROOT_DIR}/resources/reference_motion/*'),
            use_bucketed_experts=False,
            amp_expert_label_source="filename",
            amp_bucket_lin_vel_threshold=0.20,
        ...
        self.use_bucketed_experts = use_bucketed_experts
        self.amp_expert_label_source = amp_expert_label_source
        self.amp_bucket_lin_vel_threshold = amp_bucket_lin_vel_threshold
        self.amp_bucket_yaw_threshold = amp_bucket_yaw_threshold
        self.amp_bucket_yaw_lin_vel_threshold = amp_bucket_yaw_lin_vel_threshold
        ...
        for i, motion_file in enumerate(motion_files):
            ...
            trajectory_label = infer_amp_motion_mode_from_name(
                motion_file,
                lin_threshold=self.amp_bucket_lin_vel_threshold,
                yaw_threshold=self.amp_bucket_yaw_threshold,
                yaw_lin_threshold=self.amp_bucket_yaw_lin_vel_threshold,
            )
            if trajectory_label is None:
                trajectory_label = AMP_MODE_OTHER
                if self.use_bucketed_experts and self.amp_expert_label_source == "filename":
                    print(
                        "WARNING: could not infer AMP motion bucket from file name, "
                        f"using stand_other: {motion_file}"
                    )
            self.trajectory_labels.append(trajectory_label)
        ...
        self.trajectory_labels = np.array(self.trajectory_labels, dtype=np.int64)
        ...
        if self.preload_transitions:
            ...
            if self.use_bucketed_experts:
                if self.amp_expert_label_source == "filename":
                    self.preloaded_labels = self._classify_traj_idxs(traj_idxs)
                elif self.amp_expert_label_source == "velocity":
                    self.preloaded_labels = self._classify_full_frames(self.preloaded_s)
                else:
                    raise ValueError(
                        "amp_expert_label_source must be 'filename' or 'velocity', "
                        f"got {self.amp_expert_label_source}"
                    )
                self._build_preloaded_label_indices()
            print(f'Finished preloading')
        elif self.use_bucketed_experts:
            raise ValueError("Bucketed AMP experts require preload_transitions=True")

    def _classify_traj_idxs(self, traj_idxs):
        traj_idxs_tensor = torch.as_tensor(traj_idxs, dtype=torch.long, device=self.device)
        trajectory_labels = torch.as_tensor(self.trajectory_labels, dtype=torch.long, device=self.device)
        return trajectory_labels[traj_idxs_tensor]

    def _classify_full_frames(self, full_frames):
        vx = full_frames[:, self.base_lin_vel_start_idx]
        vy = full_frames[:, self.base_lin_vel_start_idx + 1]
        wz = full_frames[:, self.base_ang_vel_start_idx + 2]
        return classify_amp_motion_modes(
            vx,
            vy,
            wz,
            self.amp_bucket_lin_vel_threshold,
            self.amp_bucket_yaw_threshold,
            self.amp_bucket_yaw_lin_vel_threshold,
        )

    def _build_preloaded_label_indices(self):
        all_indices = torch.arange(self.preloaded_s.shape[0], device=self.device)
        self.preloaded_all_indices = all_indices
        self.preloaded_label_indices = {}
        bucket_parts = [
            f"source={self.amp_expert_label_source}",
            f"all={all_indices.numel()}",
        ]
        for label in AMP_MODE_IDS:
            self.preloaded_label_indices[label] = torch.nonzero(
                self.preloaded_labels == label,
                as_tuple=False,
            ).flatten()
            bucket_parts.append(
                f"{AMP_MODE_NAMES[label]}={self.preloaded_label_indices[label].numel()}"
            )
        print("AMP expert buckets: " + ", ".join(bucket_parts))

    def sample_batch_by_labels(self, labels):
        if not self.preload_transitions:
            raise RuntimeError("sample_batch_by_labels requires preload_transitions=True")
        if not hasattr(self, "preloaded_label_indices"):
            raise RuntimeError("Bucketed AMP expert indices are not initialized")

        labels = labels.to(device=self.device, dtype=torch.long)
        batch_size = labels.shape[0]
        obs_dim = self.key_body_pos_end_idx - self.base_lin_vel_start_idx
        expert_s = torch.zeros(batch_size, obs_dim, device=self.device, dtype=self.preloaded_s.dtype)
        expert_s_next = torch.zeros_like(expert_s)
        all_indices = getattr(self, "preloaded_all_indices", None)
        ...
        for label in torch.unique(labels):
            ...
            if num_samples == 0:
                continue
            pool = self.preloaded_label_indices.get(label_int, all_indices)
            if pool.numel() == 0:
                pool = all_indices
            sample_ids = pool[torch.randint(pool.numel(), (num_samples,), device=self.device)]
            expert_s[mask] = self.preloaded_s[
                sample_ids,
                self.base_lin_vel_start_idx:self.key_body_pos_end_idx,
            ]
            expert_s_next[mask] = self.preloaded_s_next[
                sample_ids,
                self.base_lin_vel_start_idx:self.key_body_pos_end_idx,
            ]
...
class MotionLoader:
    def __init__(self,
        ...
        #           # key body point positions relative to the base in the world frame, shape [motion_length, num_key_bodies, 3]
        # }
        motion_file_dir = WHEEL_LEGGED_GYM_ROOT_DIR + "/resources/reference_motion/"
