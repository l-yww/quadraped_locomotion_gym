...
class CowaCfg(LeggedRobotCfg):
    ...
    class env(LeggedRobotCfg.env):
        ...
        if projected_gravity:
            num_single_obs = 2 + 3 + 3*num_actions - 2 + 3 + 3 + 1 # cmd + dof pos[w/o wheel] + dof vel + action + imu
        else:
            num_single_obs = 2 + 3 + 3*num_actions - 2 + 3 + 2 + 1 # cmd + dof pos[w/o wheel] + dof vel + action + imu
