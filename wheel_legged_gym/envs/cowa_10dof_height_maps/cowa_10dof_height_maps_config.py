...
class CowaCfg(LeggedRobotCfg):
    ...
    class env(LeggedRobotCfg.env):
        ...
        single_num_privileged_obs = num_single_obs + num_est_prob + 1 + num_height_maps
    ...
    class domain_rand(LeggedRobotCfg.domain_rand):
        ...
        # --------------- 高度图z方向的高斯噪声、随机施加的尖端噪声、随机offset和rotation ----------------- #
        add_height_noise = True and use_random
        add_height_gaussian_noise = True and use_random
        height_gaussian_noise = 0.02
        add_height_spike_noise = True and use_random
        height_spike_noise_range = [0.1, 0.5]

        randomize_height_offset = True and use_random
        height_offset_range = [-0.02, 0.02]
        randomize_height_rotation = True and use_random
        height_rotation_roll_range = [-0.2, 0.2]
        height_rotation_pitch_range = [-0.2, 0.2]
        height_rotation_yaw_range = [-0.2, 0.2]
        ...
        add_height_lag = True and use_random
        height_lag_timesteps_range = [20, 100] 
        randomize_height_lag_timesteps = True
        randomize_height_lag_timesteps_perstep = False
    ...
    class rewards:
        ...
        soft_dof_pos_limit = (
            0.95 #1  # percentage of urdf limits, values above this limit are penalized
        )
...
class CowaCfgPPO(LeggedRobotCfgPPO):
    seed = 10
    runner_class_name = 'OnPolicyRunner_DH_Smooth_Map'   # OnPolicyRunnerEstimator
    ...
    class runner(LeggedRobotCfgPPO.runner):
        policy_class_name = 'ActorCritic_DH_Smooth_Map' #'ActorCritic_DH'
        algorithm_class_name = 'PPO_DH_Smooth_Map' #'PPO_DH'
