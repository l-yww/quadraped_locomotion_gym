...
class CowaCfg(LeggedRobotCfg):
    ...
    class env(LeggedRobotCfg.env):
        ...
        c_frame_stack = 3
        ## obs nums
        # num_single_obs = (25) + 12 + 1 + 3 + 6 + 6 + 6 + (num_height_scan_input + num_est_prob) + 2 #184
        single_num_privileged_obs = (25) + 12 + 1 + 3 + 6 + 6 + 6 + (num_height_scan_input + num_est_prob) #184
        num_single_obs = single_num_privileged_obs


        num_observations = int(frame_stack * single_num_privileged_obs)  #actor
    ...
    class normalization:
        class obs_scales:
            ...
            dof_vel = 0.05
            dof_acc = 0.0025
            height_measurements = 20.0 #5.0  ## NOTE: change big make it obvious in heights
            torques = 0.05
            quat = 1
            forces = 0.1
