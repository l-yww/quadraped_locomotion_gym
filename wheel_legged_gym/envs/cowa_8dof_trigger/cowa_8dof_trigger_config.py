...
class CowaCfg(LeggedRobotCfg):
    ...
    class env(LeggedRobotCfg.env):
        ...
        num_est_prob = 3 + 1 # vel_xyz, height, wheel_height预测的信息的总维度
        num_actions = Cowa_Num.DOF

        if projected_gravity:
            num_single_obs = 3 + 3 * num_actions - 2 + 3 + 3 # cmd + dof pos[w/o wheel] + dof vel + action + imu
        else:
            num_single_obs = 3 + 3 * num_actions - 2 + 3 + 2 # cmd + dof pos[w/o wheel] + dof vel + action + imu

        num_observations = int(frame_stack * num_single_obs)

        if projected_gravity:
            single_num_privileged_obs = num_single_obs + num_est_prob + 77
        else:
            single_num_privileged_obs = num_single_obs + num_est_prob + 1 + 77

        priv_avg_contact_force = True
        if priv_avg_contact_force:
            single_num_privileged_obs += 6
    ...
    class asset(LeggedRobotCfg.asset):
        ...
        file = f"{WHEEL_LEGGED_GYM_ROOT_DIR}/resources/robots/cowa_wheel_legged_v2/cowa_wheel_legged_v2/urdf/wheel_v2_8dof_wo_arm.urdf"
    ...
    class viewer:
        ...
        draw_volume_sample_points = True #False
    ...
    class feedforward:
        ...
        """
            ...
            右腿再swing 0.6s
        """
        swing_duration = 0.4 #0.5 #0.3 #0.6
        phase_offset = 0.4 #0.5 #0.3 #0.6
        scale = 0.2 #0.6
    ...
    class rewards:
        ...
        target_wheel_height_min = 0.01 #0.04
        target_wheel_height_max = 0.06 #0.03 #0.10
        ...
        class scales:
            ...
            #################    style rewards    ######################
            wheel_contact_number = 3 #2 #4 #5
            wheel_air_time = 6 #4 #5
            wheel_clearance = 3 #2 #5
            " 机体姿态 "
            base_height = -10 #-5 
            orientation = -20 #-10 
            " 脚部姿态 "
            ...
            same_wheel_x_position = -1
            same_wheel_z_position = -0.5 #-1
            wheel_zero_velocity = 1.0 #0.5
            ############## normalized rewards #####################
            lin_vel_z = -0.3
            ang_vel_xy = -0.3
            dof_vel = -5e-5             
            ...
            dof_pos_limits = -0.1
            dof_vel_limits = -0.1
            wheel_contact_forces = -0.01
            # wheel_stumble = -0.5 #-2
            # wheel_spin = -1
            opposite_wheel_vel = -1
            opposite_vel = -2
            # wheel_angular_vel_limit = -0.01
