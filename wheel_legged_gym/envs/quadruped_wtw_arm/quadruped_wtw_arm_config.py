from wheel_legged_gym.envs.quadruped_wtw_arm.legged_robot_config import LeggedRobotCfg, LeggedRobotCfgPPO
from wheel_legged_gym.utils.helpers import get_quadruped_arm_joint_names, get_quadruped_arm_default_joint_friction, get_quadruped_arm_default_joint_damping, get_quadruped_arm_default_joint_armature
...
class QuadWtwCfg(LeggedRobotCfg):
    ...
    class env(LeggedRobotCfg.env):
        ...
        #num_single_obs = num_commands + 2 * Cowa_Num.DOF + num_actions + timing_obs_dim # cmd + dof pos + dof vel + action + timing
        observe_upper_body = False  #观测不包括机械臂电机
        if observe_upper_body: num_single_obs = num_commands + 2 * Cowa_Num.DOF + num_actions + timing_obs_dim # cmd + dof pos + dof vel + action + timing
        else: num_single_obs = num_commands + 2 * num_lower_dof + num_actions + timing_obs_dim # cmd + dof pos + dof vel + action + timing
        ...
        if priv_observe_motor_strength:
            single_num_privileged_obs += Cowa_Num.DOF
        ...
        action_curriculum = False  #加入机械臂动作课程学习

    class asset(LeggedRobotCfg.asset):
        DOF = Cowa_Num.DOF
        WHEEL_LEGGED_GYM_ROOT_DIR = "{WHEEL_LEGGED_GYM_ROOT_DIR}"
        file = f"{WHEEL_LEGGED_GYM_ROOT_DIR}/resources/robots/cowa_quadruped_arm_v1/urdf/cowa_quadruped_arm_v1.urdf"
    ...
    class init_state(LeggedRobotCfg.init_state):
        ...
        joint_names = get_quadruped_arm_joint_names()
        default_joint_angles = {name: 0.0 for name in joint_names}
        # 固定臂关节角度
        default_joint_angles["arm17"] = -0.0
        default_joint_angles["arm18"] = -0.61
        default_joint_angles["arm19"] = -3.05
        default_joint_angles["arm20"] = -0.0
        default_joint_angles["arm21"] =  0.87
        default_joint_angles["arm22"] = -0.0
    ...
    class domain_rand(LeggedRobotCfg.domain_rand):
        ...
        upper_interval_s = 1     # 每隔1秒重新采样上肢目标
        init_upper_ratio = 0.    # 初始扰动比例=0（无扰动）
        ...
        com_displacement_range = [-0.1, 0.1]  # base link com的随机化范围
        ...
        default_joint_friction = get_quadruped_arm_default_joint_friction(Cowa_Num.quad_index)
        ...
        default_joint_damping = get_quadruped_arm_default_joint_damping(Cowa_Num.quad_index)
        ...
        default_joint_armature = get_quadruped_arm_default_joint_armature(Cowa_Num.quad_index)
    ...
    class commands(LeggedRobotCfg.commands):
        ...
        class ranges:
            ...
            heading = [-3.14, 3.14]
            body_height_cmd = [0.30, 0.45]           # 目标高度
            gait_phase_cmd_range = [0.5, 0.5]        # trot:对角腿同相
            gait_offset_cmd_range = [0.0, 0.0]       # trot
            gait_bound_cmd_range = [0.0, 0.0]        # trot
            gait_frequency_cmd_range = [2, 2]    # 步频
            gait_duration_cmd_range = [0.5, 0.5]   # 50% 占空比，让狗学会在跑和走之间过渡
            footswing_height_range = [0.05, 0.15]
    ...
    class rewards:
        ...
        class scales:
            ...
            tracking_ang_vel = 5  #1
            #################    style rewards    ######################
            " 机体姿态 "
            base_height = -10 #-10
            #orientation = -10 #-20
            orientation_control = 3.0  #动态保证base水平

            tracking_contacts_shaped_force = -1.0 #3.0
            tracking_contacts_shaped_vel = -1.0 #3.0
            feet_clearance_cmd_linear = -1.0 #-2.0 #-5.0
            #feet_clearance_cmd_exp = 1.0  #奖励抬腿

            ############## normalized rewards #####################
            " 静止稳定 "
            stand_base_vel_penality = -3.0   #-1.0
            stand_stability = -3.0           #-1.0
            stand_all_feet_contact = 4.0     # 2.0

            " 端正站姿 "
            dof_pos_symmetry= -2.0   #-1.0 
