import numpy as np
import torch
from wheel_legged_gym.envs.cowa_wbc_stage.legged_robot_config import LeggedRobotCfg, LeggedRobotCfgPPO
from wheel_legged_gym.utils.helpers_stage import get_joint_names, get_stiffness_damping,\
...
class CowaCfg(LeggedRobotCfg):
    ...
    class env(LeggedRobotCfg.env):
        ...
        num_est_prob_arm = 3
        num_est_prob_leg = 3 + 1
        num_est_prob = num_est_prob_arm + num_est_prob_leg

        num_single_obs_arm = 3 + 9 + 6 + 6 + 6  # 30
        num_single_obs_leg = 3 + 8 + 10 + 10 + 3 + 2  # 36

        num_single_obs = 3 + 3 + 9 + 14 + 16 + 16 + 3 + 2 # 66

        num_privileged_obs_arm = num_single_obs_arm + num_est_prob_arm  # 33
        num_privileged_obs_leg = num_single_obs_leg + num_est_prob_leg + 1 # 54

        num_privileged_obs = num_single_obs + num_est_prob + 1

        num_observations = int(frame_stack * num_single_obs)
        num_privileged_obs = int(c_frame_stack * num_privileged_obs)
    ...
    class commands(LeggedRobotCfg.commands):
        ...
        T_traj = [4.0,6.0]  # 轨迹周期范围（秒）[2.5,3.0]

        tracking_start_step = 5000
        ...
        class ranges:
            ...
            pos_l = [0.45, 0.8]
            pos_p = [0.5*torch.pi, 0.85*torch.pi]
            pos_y = [-torch.pi, torch.pi]

            # pos_l = [0.6, 0.7]
            # pos_p = [0.1*torch.pi, 0.2*torch.pi]
            # pos_y = [0, 0]
            # pos_l = [0.45, 0.45]
            # pos_p = [-1.0, -1.0]
    ...
    class rewards:
        ...
        class scales:
            ...
            dof_pos_limits = -0.1
            dof_vel_limits = -0.1
            # stand_still = -0.01 #-0.2 #-0.4
            stand_base_vel_penality = -2
            # feet_contact_forces = -0.01
            feet_ground_parallel = -5
            # feet_stumble = -2
...
class CowaCfgPPO(LeggedRobotCfgPPO):
    seed = 10
    runner_class_name = 'OnPolicyRunner_DH_Smooth_Stage'   # OnPolicyRunnerEstimator
    ...
    class runner(LeggedRobotCfgPPO.runner):
        policy_class_name = 'ActorCritic_DH_Smooth_Stage' #'ActorCritic_DH'
        algorithm_class_name = 'PPO_DH_Smooth_Stage' #'PPO_DH'
