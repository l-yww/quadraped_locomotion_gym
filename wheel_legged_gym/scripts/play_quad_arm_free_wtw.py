...
import json
import copy
from wheel_legged_gym.utils.Integrated import Integrated_vallina_policy
...
def play(args):
    ...
    if EXPORT_POLICY:
        ...
        if args.task == 'quadruped' or args.task == 'quadruped_wtw_arm':
            ...
            print('Exported as PT to:', os.path.join(path, f"{export_name}.pt"))
            #integrated_policy.export_onnx(path, export_name)
            #print('Exported as ONNX to:', os.path.join(path, f"{export_name}.onnx"))
...
if __name__ == '__main__':
    ...
    args.task= 'quadruped_wtw_arm'
    args.experiment_name = args.task
    args.load_run = 'arm固定-加强静止'
    #args.load_run = 'test'
    # args.load_run = '抬腿正0.10cm-sin-pitch'
    args.checkpoint = 122600
