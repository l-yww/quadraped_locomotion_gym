...
import json
import copy
from wheel_legged_gym.utils.Integrated import Integrated_vallina_policy,Integrated_DH_policy
...
def handle_keyboard(env):
    ...
    for evt in env.gym.query_viewer_action_events(env.viewer):
        ...
        if action == "CMD_VX_UP":
            ...
            COMMAND_YAW = max(COMMAND_YAW - 0.1, -1.0)
        elif action == "CMD_YAW_RIGHT":
            COMMAND_YAW = min(COMMAND_YAW + 0.1, 1.0)
        elif action == "CMD_HEIGHT_UP":
            COMMAND_BODY_HEIGHT = min(COMMAND_BODY_HEIGHT + 0.05, 0.30)
        elif action == "CMD_HEIGHT_DOWN":
            COMMAND_BODY_HEIGHT = max(COMMAND_BODY_HEIGHT - 0.05, 0.45)
        elif action == "CMD_FREQ_UP":
            COMMAND_GAIT_FREQ = min(COMMAND_GAIT_FREQ + 0.5, 2.0)
...
def play(args):
    ...
    if EXPORT_POLICY:
        ...
        # if args.task == 'quadruped' or args.task == 'quadruped_wtw':
        ...
        if args.task in ['quadruped', 'quadruped_wtw', 'quadruped_wtw_arm_dh']:
            # integrated_policy = Integrated_vallina_policy(ppo_runner.alg.actor_critic.actor,
            #                                         env_cfg.env.num_single_obs, env_cfg.env.frame_stack, env_cfg.env.actor_input_stack)
            integrated_policy = Integrated_DH_policy(
                ppo_runner.alg.actor_critic.actor,
                ppo_runner.alg.actor_critic.estimator,
                ppo_runner.alg.actor_critic.long_history,
                ...
                env_cfg.env.num_single_obs,  # in_channels
                env_cfg.env.num_single_obs,  # num_proprio_obs
...
if __name__ == '__main__':
    ...
    args.task= 'quadruped_wtw_arm_dh'
    args.experiment_name = args.task
    args.load_run = 'quadruped_wtw_arm_dh_test'
    #args.load_run = 'test'
    # args.load_run = '抬腿正0.10cm-sin-pitch'
    args.checkpoint = 167000
