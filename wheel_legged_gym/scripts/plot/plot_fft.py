...
sampling_period = 1 / sampling_frequency  # 采样周期（秒）

data = actions_data_transposed[0:1]

data_transposed = list(zip(*data))

n = len(data_transposed)

ft = np.fft.fft(data_transposed)
freq = np.fft.fftfreq(n, d=sampling_period)
plt.figure(figsize=(10, 6))
plt.plot(freq[:n//2], np.abs(ft)[:n//2], label='FFT of dof')
plt.xlabel('Frequency (Hz)')
plt.ylabel('Amplitude')
plt.title('FFT Analysis of dof')
plt.legend()
plt.grid(True)
# plt.xlim(-0.1, 2000) 
plt.show()

data = actions_data_transposed[2:3]
...
data = actions_data_transposed[1:2]
...
data = actions_data_transposed[3:4]
