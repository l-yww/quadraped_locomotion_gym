...
from wheel_legged_gym.utils import  get_args, export_policy_as_jit, task_registry_stage
from wheel_legged_gym.utils import Logger, Logger_pd, Logger_foot, Logger_wbc
...
def play(args):
    ...
    env_cfg, train_cfg = task_registry_stage.get_cfgs(name=args.task)
    ...
    env_cfg.commands.tracking_ee_start_step = 0 #10000
    env_cfg.commands.force_start_step = 0
    env_cfg.commands.force_end_step = 0
    ...
    env_cfg.viewer.debug_viz = False #True
    ...
    # prepare environment
    env, _ = task_registry_stage.make_env(name=args.task, args=args, env_cfg=env_cfg)
    ...
    ppo_runner, train_cfg = task_registry_stage.make_alg_runner(env=env, name=args.task, args=args, train_cfg=train_cfg)
    policy_arm, policy_leg = ppo_runner.get_inference_policy(device=env.device)
    ...
    if EXPORT_POLICY:
        ...
        if "him" in args.task and "terrain" in args.task: 
            ...
            integrated_policy = Integrated_TS_policy(ppo_runner.alg.actor_critic.actor, # actor
                                                ...
                                                env_cfg.env.num_single_obs, env_cfg.env.frame_stack)
            integrated_policy.export(path, args.load_run)  
        elif args.task == 'cowa_dh' or args.task == 'cowa_4dof' or args.task == 'cowa_8dof_plane' or args.task == 'cowa_10dof' or args.task == 'cowa_10dof_p3o' or args.task == 'cowa_wbc':
            ...
            integrated_policy = Integrated_DH_Map_policy(ppo_runner.alg.actor_critic.actor, # actor
                                                ...
                                                ppo_runner.alg.actor_critic.num_proprio_obs,
                                                train_cfg.policy.long_history_type)
        elif args.task == 'cowa_wbc_stage':
            integrated_policy = Integrated_DH_policy(ppo_runner.alg.actor_critic_leg.actor, # actor
                                                ppo_runner.alg.actor_critic_leg.estimator, # est
                                                ppo_runner.alg.actor_critic_leg.long_history, # long history_encoder
                                                env_cfg.env.num_single_obs_leg, 
                                                env_cfg.env.frame_stack,
                                                env_cfg.env.short_frame_stack,
                                                # ppo_runner.alg.actor_critic.in_channels,
                                                ppo_runner.alg.actor_critic_leg.history_len,
                                                ppo_runner.alg.actor_critic_leg.num_proprio_obs,
                                                train_cfg.policy.long_history_type)
        integrated_policy.export(path, args.load_run)
        print('Exported policy as jit script to: ', path)

    logger = Logger_wbc(env.dt)
    # print(env.dt)
    ...
    if RENDER:
        ...
        camera_offset = gymapi.Vec3(1, -1, 1.2)
    ...
    for i in range(stop_state_log * 10):
        ...
        elif "cowa_wbc_stage" in args.task:
            actions = torch.zeros([env.num_envs,env.num_actions],device=env.device)
            obs_history = obs.clone().reshape(env.num_envs, -1, env.cfg.env.num_single_obs)
            arm_actor_obs = torch.cat([   
                                   obs_history[:,:,3:15],   # arm cmd
                                   obs_history[:,:,15:21],  # arm pos
                                   obs_history[:,:,29:35],  # arm vel
                                   obs_history[:,:,45:51]],dim=2).reshape(env.num_envs,-1).detach() # arm action
            leg_actor_obs = torch.cat([obs_history[:,:,:3],                             # leg cmd
                                    obs_history[:,:,21:29],                           # all pos
                                    obs_history[:,:,35:45],                          # all vel
                                    obs_history[:,:,51:61],                          # leg action
                                    obs_history[:,:,61:66]],dim=2).reshape(env.num_envs,-1).detach() 
            actions[:,:6] = policy_arm(arm_actor_obs) 
            # print("arm_actions", actions[:,:6])
            actions[:,6:] = policy_leg(leg_actor_obs)
        else:
        ...
        logger.log_states(
            {
                ...
                # NOTE torque
                "joint1_torque": env.torques[robot_index, 0].item(),
                "joint2_torque": env.torques[robot_index, 1].item(),
                "joint3_torque": env.torques[robot_index, 2].item(),
                "joint4_torque": env.torques[robot_index, 3].item(),
                "joint5_torque": env.torques[robot_index, 4].item(),
                "joint6_torque": env.torques[robot_index, 5].item(),
                "left_hip_roll_torque": env.torques[robot_index, 6].item(),
                "left_hip_pitch_torque": env.torques[robot_index, 7].item(),
                "left_knee_torque": env.torques[robot_index, 8].item(),
                "left_foot_torque": env.torques[robot_index, 9].item(),
                "left_wheel_torque": env.torques[robot_index, 10].item(),
                "right_hip_roll_torque": env.torques[robot_index, 11].item(),
                "right_hip_pitch_torque": env.torques[robot_index, 12].item(),
                "right_knee_torque": env.torques[robot_index, 13].item(),
                "right_foot_torque": env.torques[robot_index, 14].item(),
                "right_wheel_torque": env.torques[robot_index, 15].item(),

                # NOTE real vel
                "joint1_vel": env.dof_vel[robot_index, 0].item(),
                "joint2_vel": env.dof_vel[robot_index, 1].item(),
                "joint3_vel": env.dof_vel[robot_index, 2].item(),
                "joint4_vel": env.dof_vel[robot_index, 3].item(),
                "joint5_vel": env.dof_vel[robot_index, 4].item(),
                "joint6_vel": env.dof_vel[robot_index, 5].item(),
                "left_hip_roll_vel": env.dof_vel[robot_index, 6].item(),
                "left_hip_pitch_vel": env.dof_vel[robot_index, 7].item(),
                "left_knee_vel": env.dof_vel[robot_index, 8].item(),
                "left_foot_vel": env.dof_vel[robot_index, 9].item(),
                "left_wheel_vel": env.dof_vel[robot_index, 10].item(),
                "right_hip_roll_vel": env.dof_vel[robot_index, 11].item(),
                "right_hip_pitch_vel": env.dof_vel[robot_index, 12].item(),
                "right_knee_vel": env.dof_vel[robot_index, 13].item(),
                "right_foot_vel": env.dof_vel[robot_index, 14].item(),
                "right_wheel_vel": env.dof_vel[robot_index, 15].item(),

                # NOTE  real pos
                "joint1_pos": env.dof_pos[robot_index, 0].item(),
                "joint2_pos": env.dof_pos[robot_index, 1].item(),
                "joint3_pos": env.dof_pos[robot_index, 2].item(),
                "joint4_pos": env.dof_pos[robot_index, 3].item(),
                "joint5_pos": env.dof_pos[robot_index, 4].item(),
                "joint6_pos": env.dof_pos[robot_index, 5].item(),
                "left_hip_roll_pos": env.dof_pos[robot_index, 6].item(),
                "left_hip_pitch_pos": env.dof_pos[robot_index, 7].item(),
                "left_knee_pos": env.dof_pos[robot_index, 8].item(),
                "left_foot_pos": env.dof_pos[robot_index, 9].item(),
                "left_wheel_pos": env.dof_pos[robot_index, 10].item(),
                "right_hip_roll_pos": env.dof_pos[robot_index, 11].item(),
                "right_hip_pitch_pos": env.dof_pos[robot_index, 12].item(),
                "right_knee_pos": env.dof_pos[robot_index, 13].item(),
                "right_foot_pos": env.dof_pos[robot_index, 14].item(),
                "right_wheel_pos": env.dof_pos[robot_index, 15].item(),
                ...
                # NOTE action
                "joint1_action": env.actions[robot_index, 0].item() * env_cfg.control.pos_action_scale,
                "joint2_action": env.actions[robot_index, 1].item() * env_cfg.control.pos_action_scale,
                "joint3_action": env.actions[robot_index, 2].item() * env_cfg.control.pos_action_scale,
                "joint4_action": env.actions[robot_index, 3].item() * env_cfg.control.pos_action_scale,
                "joint5_action": env.actions[robot_index, 4].item() * env_cfg.control.pos_action_scale,
                "joint6_action": env.actions[robot_index, 5].item() * env_cfg.control.pos_action_scale,
                "left_hip_roll_action": env.actions[robot_index, 6].item() * env_cfg.control.pos_action_scale,
                "left_hip_pitch_action": env.actions[robot_index, 7].item() * env_cfg.control.pos_action_scale,
                "left_knee_action": env.actions[robot_index, 8].item() * env_cfg.control.pos_action_scale,
                "left_foot_action": env.actions[robot_index, 9].item() * env_cfg.control.pos_action_scale,
                "left_wheel_action": env.actions[robot_index, 10].item() * env_cfg.control.pos_action_scale,
                "right_hip_roll_action": env.actions[robot_index, 11].item() * env_cfg.control.pos_action_scale,
                "right_hip_pitch_action": env.actions[robot_index, 12].item() * env_cfg.control.pos_action_scale,
                "right_knee_action": env.actions[robot_index, 13].item() * env_cfg.control.pos_action_scale,
                "right_foot_action": env.actions[robot_index, 14].item() * env_cfg.control.pos_action_scale,
                "right_wheel_action": env.actions[robot_index, 15].item() * env_cfg.control.pos_action_scale,
                ...
                # NOTE real power
                "joint1_power": (env.torques[robot_index, 0] * env.dof_vel[robot_index, 0]).item(),
                "joint2_power": (env.torques[robot_index, 1] * env.dof_vel[robot_index, 1]).item(),
                "joint3_power": (env.torques[robot_index, 2] * env.dof_vel[robot_index, 2]).item(),
                "joint4_power": (env.torques[robot_index, 3] * env.dof_vel[robot_index, 3]).item(),
                "joint5_power": (env.torques[robot_index, 4] * env.dof_vel[robot_index, 4]).item(),
                "joint6_power": (env.torques[robot_index, 5] * env.dof_vel[robot_index, 5]).item(),
                "left_hip_roll_power": (env.torques[robot_index, 6] * env.dof_vel[robot_index, 6]).item(),
                "left_hip_pitch_power": (env.torques[robot_index, 7] * env.dof_vel[robot_index, 7]).item(),
                "left_knee_power": (env.torques[robot_index, 8] * env.dof_vel[robot_index, 8]).item(),
                "left_foot_power": (env.torques[robot_index, 9] * env.dof_vel[robot_index, 9]).item(),
                "left_wheel_power": (env.torques[robot_index, 10] * env.dof_vel[robot_index, 10]).item(),
                "right_hip_roll_power": (env.torques[robot_index, 11] * env.dof_vel[robot_index, 11]).item(),
                "right_hip_pitch_power": (env.torques[robot_index, 12] * env.dof_vel[robot_index, 12]).item(),
                "right_knee_power": (env.torques[robot_index, 13] * env.dof_vel[robot_index, 13]).item(),
                "right_foot_power": (env.torques[robot_index, 14] * env.dof_vel[robot_index, 14]).item(),
                "right_wheel_power": (env.torques[robot_index, 15] * env.dof_vel[robot_index, 15]).item(),
...
if __name__ == '__main__':
    ...
    args.task= 'cowa_wbc_stage' #'cowa_dh'
    args.experiment_name = args.task
    args.load_run = "Apr04_11-19-23_v2_wbc_stage_[command_set_maker]_[w_DR]_[same_foot_x_position=-0.1]"
    args.checkpoint = -1
