...
params = {
    ...
    "vy": [-0.4, 0.4],
    "height": [0.25, 0.38],
    "vx_offset": -0.03, # vx速度偏置
    "vy_offset": 0.0, # vy速度偏置

    # 手臂控制
    "arm_state": False, # 观测是否拿手臂信息
...
def run_policy(policy, cfg, observation_data):
    ...
    action = np.zeros(6, dtype=np.double)
    ...
    while(1):
        ...
        # elif abs(last_cmd_vx) > 1e-6 and abs(cmd.vx) > 1e-6:
        #     odom_pos_data = [0.,0.]
        if abs(motion_cmd['speed']) > 0.01 or abs(motion_cmd['steer']) > 0.01:
        ...
        with torch.no_grad():
           try:
                action[:] = policy(all_obs_tensor)[0].cpu().numpy()[:6]
        ...
        if (print_count < 50):
            ...
            log.write_info(f'-----------action----------')
            log.write_info(f'action:{action} count:{print_count}')
        ...
        if (time.time() - last_time > 1):
            print("vx: {:.3f} vy: {:.3f} vz: {:.3f} yawinit: {:.4f} yaw: {:.4f} heading_error: {:.4f} dyaw_raw: {:.4f}"\
                  .format(cmd.vx,cmd.dyaw,cmd.height,imu_init_yaw,eu_ang[2],heading_error,motion_cmd['steer']))
            #print("vx: {:.3f} vy: {:.3f} vz: {:.3f} yawinit: {:.4f} yaw: {:.4f}".format(cmd.vx,cmd.dyaw,cmd.height,imu_init_yaw,eu_ang[2]))
            #print(':',q_arm[1:])
            print('odom:',odom_gap)
            log.write_info(f'vx: {cmd.vx} vy: {cmd.dyaw} vz: {cmd.height} yawinit: {imu_init_yaw} yaw: {eu_ang[2]} heading_error: {heading_error} dyaw_raw: {motion_cmd["steer"]}')
            log.write_info(f'odom:{odom_gap}')
...
if __name__ == '__main__':
    ...
    if not os.path.isfile(args.load_model):
        ...
        log.write_info(f'Error: Model file {args.load_model} does not exist.')
        sys.exit(1)
    ...
    try:
        ...
        log.write_info(f'Failed to load model: {e}')
        sys.exit(1)
