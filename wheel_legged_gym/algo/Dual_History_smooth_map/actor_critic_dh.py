...
class ActorCritic_DH_Smooth_Map(nn.Module):
    def __init__(self,  num_short_obs,
                        ...
                        stride_size=[3, 2],
                        lh_output_dim=64,
                        height_history_len = 5,
                        height_kernel_size = [3, 3],
                        height_stride_size = [1, 1],
                        height_filter_size = [16, 8],
                        height_output_dim=64,
                        init_noise_std=1.0,
                        activation = nn.ELU(),
                        **kwargs):
        if kwargs:
            print("ActorCritic.__init__ got unexpected arguments, which will be ignored: " + str([key for key in kwargs.keys()]))
        super(ActorCritic_DH_Smooth_Map, self).__init__()

        self.num_short_obs = num_short_obs
        self.num_est_prob = num_est_prob
        self.lh_output_dim = lh_output_dim
        self.height_output_dim = height_output_dim
        self.num_proprio_obs = num_single_obs
        self.num_height_maps = num_height_maps
        mlp_input_dim_a = num_short_obs + self.num_est_prob + self.lh_output_dim + self.height_output_dim
        ...
        #define height_maps CNN
        height_history_layers = []
        height_in_channels = self.num_height_maps
        self.height_history_len = height_history_len
        height_cnn_output_dim = self.height_history_len
        for height_out_channels, height_kernel_size, height_stride_size in zip(height_filter_size, height_kernel_size, height_stride_size):
            height_history_layers.append(nn.Conv1d(in_channels=height_in_channels, out_channels=height_out_channels, kernel_size=height_kernel_size, stride=height_stride_size))
            height_history_layers.append(nn.ReLU())
            height_cnn_output_dim = (height_cnn_output_dim - height_kernel_size + height_stride_size) // height_stride_size
            height_in_channels = height_out_channels
        height_cnn_output_dim *= height_out_channels
        height_history_layers.append(nn.Flatten())
        height_history_layers.append(nn.Linear(height_cnn_output_dim, 128))
        height_history_layers.append(nn.ELU())
        height_history_layers.append(nn.Linear(128, height_output_dim))
        self.height_history = nn.Sequential(*height_history_layers)

        print(f"Actor MLP: {self.actor}")
        print(f"Critic MLP: {self.critic}")
        print(f"Estimator MLP: {self.estimator}")
        print(f"long_history CNN: {self.long_history}")
        print(f"height_history CNN: {self.height_history}")
    ...
    def act(self, observations, height_maps, **kwargs):
        ...
        compressed_height_history = self.height_history(height_maps.view(-1, self.num_height_maps, self.height_history_len))
    ...
    def act_inference(self, observations, height_maps):
        short_history = observations[...,-self.num_short_obs:]
        estimated_prob = self.estimator(short_history)
        compressed_long_history = self.long_history(observations.view(-1, self.num_proprio_obs, self.history_len))
        compressed_height_history = self.height_history(height_maps.view(-1, self.num_height_maps, self.height_history_len))
