...
class ActorCritic_DH_prop(nn.Module):
    def __init__(self,  num_short_obs,
        ...
        super(ActorCritic_DH_prop, self).__init__()
        ...
        mlp_input_dim_a = num_short_obs + self.num_est_prob + self.lh_output_dim + 3
    ...
    def act_inference(self, observations, commands):
        short_history = observations[...,-self.num_short_obs:]
        estimated_prob = self.estimator(short_history)
        compressed_long_history = self.long_history(observations.view(-1, self.in_channels, self.num_proprio_obs))
        actor_obs = torch.cat((commands, short_history, estimated_prob, compressed_long_history),dim=-1)
