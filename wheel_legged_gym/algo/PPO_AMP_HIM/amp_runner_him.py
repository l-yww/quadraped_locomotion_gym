...
from wheel_legged_gym.algo.PPO_AMP_HIM.ppo_amp_him import PPO_AMP_HIM
from wheel_legged_gym.algo.PPO.actor_critic import ActorCritic
from wheel_legged_gym.algo.HIM.actor_critic_him import ActorCritic_HIM
from wheel_legged_gym.algo.HIM.actor_critic_him_heightmap import ActorCritic_HIM_Heightmap
...
from wheel_legged_gym.algo.PPO_AMP.amp_runner import AMPRunner
...
class AMPRunner_HIM(AMPRunner):
    ...
    def __init__(
        ...
        # 与 HIM runner 对齐: 终端打印前缀 [task/run] [start_time]
    ...
    def _init_agent_and_algo(self) -> None:
        ...
        # ActorCritic_HIM / ActorCritic_HIM_Heightmap 都需要额外参数:
        # num_short_obs, num_single_obs, num_est_prob (Heightmap 版另需 num_body_dim, 经 policy_cfg 透传)
        if self.cfg["policy_class_name"] in ("ActorCritic_HIM", "ActorCritic_HIM_Heightmap"):
            # ActorCritic_HIM 需要额外参数: num_short_obs, num_single_obs, num_est_prob
        ...
        # 用 compute_symmetric_states_d1 (URDF 顺序 + HISTORY_LEN 自适应 frame_stack)
        # symmetry_scale>0 即启用 mirror loss + 数据增强
        configured_symmetry_cfg = alg_cfg.pop("symmetry_cfg", None)
        symmetry_scale = float(alg_cfg.pop("symmetry_scale", 0.0))

        if configured_symmetry_cfg is not None:
            symmetry_cfg = configured_symmetry_cfg

        elif symmetry_scale > 0:
            symmetry_cfg = {
                "use_data_augmentation": True,           # 数据增强: batch 翻倍 (原+镜像)
                "data_augmentation_func": compute_symmetric_states_d1,
                "use_mirror_loss": True,
                "mirror_loss_coeff": symmetry_scale,     # 用 symmetry_scale 当 mirror loss 系数
            }
        else:
            symmetry_cfg = None
    ...
    def learn(
        ...
        # 检查是否有 estimator（ActorCritic_HIM 才有）
